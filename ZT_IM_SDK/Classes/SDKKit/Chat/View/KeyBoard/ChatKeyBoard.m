...
- (void)updateAssociateTableViewFrame
{
    ...
    //更新表的frame
    _associateTableView.frame = CGRectMake(0, _associateTableView.frame.origin.y, self.frame.size.width, self.frame.origin.y - _associateTableView.frame.origin.y);
    ...
    if (_associateTableView.contentSize.height +_associateTableView.contentInset.top+_associateTableView.contentInset.bottom> _associateTableView.frame.size.height) {
        NSInteger sections = [_associateTableView numberOfSections];
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:sections - 1];
        [_associateTableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionBottom animated:YES];
    }
}
#pragma mark -- kvo
