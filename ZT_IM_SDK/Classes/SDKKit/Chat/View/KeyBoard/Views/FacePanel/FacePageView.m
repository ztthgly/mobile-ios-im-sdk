...
//    ZT_Log(@"prepareForReuse------------------------------------------");
//    ZT_Log(@"%zd", self.themeStyle);
}
...
- (void)loadSystemEmoji:(NSArray *)faceData
{
    ...
    [btn setImage:UIImageMake(@"Delete_ios7") forState:UIControlStateNormal];
...
- (void)loadCustomEmoji:(NSArray *)faceData
{
    for (int i = 0; i < faceData.count; i++) {
        ...
        FaceButton *btn = self.buttons[i];
        btn.hidden = NO;
        [btn setImage:UIImageMake(fm.faceIcon) forState:UIControlStateNormal];
        btn.faceTitle = fm.faceTitle;
    }
